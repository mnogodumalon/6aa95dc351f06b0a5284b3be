/**
 * PlatzkontingenteDialog — pre-generated create/edit dialog for Platzkontingente.
 *
 ...
 * PlatzkontingenteDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Platzkontingente['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<PlatzkontingenteDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Platzkontingente, Einrichtungen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Platzkontingente';
...
/** Widened prefill type for PlatzkontingenteDialog.defaultValues — see file header. */
export type PlatzkontingenteDialogDefaults = Omit<Platzkontingente['fields'], 'betreuungsform'> & {
    betreuungsform?: LookupValue | string;
  };

interface PlatzkontingenteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Platzkontingente['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: PlatzkontingenteDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  betreuungsform: LOOKUP_OPTIONS['platzkontingente']?.['betreuungsform'] ?? [],
};
...
export function PlatzkontingenteDialog({ open, onClose, onSubmit, defaultValues, recordId, einrichtungenList, enablePhotoScan = true, enablePhotoLocation = true }: PlatzkontingenteDialogProps) {
  const [fields, setFields] = useState<Partial<Platzkontingente['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['einrichtung', 'kita_jahr', 'betreuungsform', 'plaetze_gesamt'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Platzkontingente['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'platzkontingente');
      await onSubmit(clean as Platzkontingente['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "einrichtung": string | null, // Display name from Einrichtungen (see <available-records>)\n  "kita_jahr": string | null, // Kita-Jahr\n  "betreuungsform": LookupValue | null, // Betreuungsform (select one key: "kindergarten" | "hort" | "krippe") mapping: kindergarten=Kindergarten (3–6 Jahre), hort=Hort, krippe=Krippe (unter 3 Jahre)\n  "plaetze_gesamt": number | null, // Plätze gesamt\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["einrichtung"]);
        ...
        return merged as Partial<Platzkontingente['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('platzkontingente') })
    : t('new_entity', { entity: appLabel('platzkontingente') });
    ...
    'einrichtung': (
      <div key="einrichtung" className="space-y-1.5">
        ...
        {showErrors && !fields.einrichtung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('platzkontingente', 'einrichtung')}</p>
        )}
      </div>
    ),
    'kita_jahr': (
      <div key="kita_jahr" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.kita_jahr && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('platzkontingente', 'kita_jahr')}</p>
        )}
      </div>
    ),
    'betreuungsform': (
      <div key="betreuungsform" className="space-y-1.5">
        <Label htmlFor="betreuungsform">{fieldLabel('platzkontingente', 'betreuungsform')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('platzkontingente', 'betreuungsform', 'kindergarten') ?? 'Kindergarten (3–6 Jahre)'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('platzkontingente', 'betreuungsform', 'hort') ?? 'Hort'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('platzkontingente', 'betreuungsform', 'krippe') ?? 'Krippe (unter 3 Jahre)'}
          </button>
        </div>
        {showErrors && !fields.betreuungsform && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('platzkontingente', 'betreuungsform')}</p>
        )}
      </div>
    ),
    'plaetze_gesamt': (
      <div key="plaetze_gesamt" className="space-y-1.5">
        <Label htmlFor="plaetze_gesamt">{fieldLabel('platzkontingente', 'plaetze_gesamt')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="plaetze_gesamt"
          type="number"
          inputMode="decimal"
        ...
        />
        {showErrors && !fields.plaetze_gesamt && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('platzkontingente', 'plaetze_gesamt')}</p>
        )}
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"einrichtung": "Einrichtung", "kita_jahr": "Kita-Jahr", "betreuungsform": "Betreuungsform", "plaetze_gesamt": "Plätze gesamt"};
  const CURRENCY_KEYS = new Set<string>(["plaetze_gesamt"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"einrichtung": {"name": "Name der Einrichtung", "traegerart": "Trägerart", "beschreibung": "Beschreibung", "foto": "Foto der Einrichtung", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "telefon": "Telefon", "email": "E-Mail", "betreuungsformen": "Betreuungsformen", "oeffnungszeiten": "Öffnungszeiten"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PLATZKONTINGENTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
