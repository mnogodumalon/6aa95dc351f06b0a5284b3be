/**
 * KinderDialog — pre-generated create/edit dialog for Kinder.
 *
 ...
 * KinderDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Kinder['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<KinderDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Kinder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kinder';
...
/** Widened prefill type for KinderDialog.defaultValues — see file header. */
export type KinderDialogDefaults = Omit<Kinder['fields'], 'geschlecht'> & {
    geschlecht?: LookupValue | string;
  };

interface KinderDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kinder['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: KinderDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  geschlecht: LOOKUP_OPTIONS['kinder']?.['geschlecht'] ?? [],
};
...
export function KinderDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KinderDialogProps) {
  const [fields, setFields] = useState<Partial<Kinder['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'geburtsdatum', 'strasse', 'hausnummer', 'plz', 'ort'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Kinder['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kinder');
      await onSubmit(clean as Kinder['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "weiblich" | "maennlich" | "divers") mapping: weiblich=Weiblich, maennlich=Männlich, divers=Divers\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "plz": string | null, // Postleitzahl\n  "ort": string | null, // Ort\n  "besonderheiten": string | null, // Besonderheiten (Allergien, Förderbedarf)\n  "geschwisterkind": boolean | null, // Geschwisterkind bereits in einer Einrichtung\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kinder['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('kinder') })
    : t('new_entity', { entity: appLabel('kinder') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'nachname')}</p>
        )}
      </div>
    ),
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="geburtsdatum"
          placeholder="Wann wurde das Kind geboren?"
          mode="date"
        ...
        {showErrors && !fields.geburtsdatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'geburtsdatum')}</p>
        )}
      </div>
    ),
    'geschlecht': (
      <div key="geschlecht" className="space-y-1.5">
        <Label htmlFor="geschlecht">{fieldLabel('kinder', 'geschlecht')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('kinder', 'geschlecht', 'weiblich') ?? 'Weiblich'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('kinder', 'geschlecht', 'maennlich') ?? 'Männlich'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.geschlecht) === 'divers'}
            onClick={() => setFields(f => ({ ...f, geschlecht: (lookupKey(f.geschlecht) === 'divers' ? undefined : 'divers') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.geschlecht) === 'divers'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('kinder', 'geschlecht', 'divers') ?? 'Divers'}
          </button>
        </div>
      </div>
    ),
    'strasse': (
      <div key="strasse" className="space-y-1.5">
        ...
        {showErrors && !fields.strasse && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'strasse')}</p>
        )}
      </div>
    ),
    'hausnummer': (
      <div key="hausnummer" className="space-y-1.5">
        ...
        {showErrors && !fields.hausnummer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'hausnummer')}</p>
        )}
      </div>
    ),
    'plz': (
      <div key="plz" className="space-y-1.5">
        ...
        {showErrors && !fields.plz && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'plz')}</p>
        )}
      </div>
    ),
    'ort': (
      <div key="ort" className="space-y-1.5">
        ...
        {showErrors && !fields.ort && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kinder', 'ort')}</p>
        )}
      </div>
    ),
    'besonderheiten': (
      <div key="besonderheiten" className="space-y-1.5">
        ...
        <Textarea
          id="besonderheiten"
          placeholder="Allergien, Förderbedarf, besondere Hinweise"
    ...
    'geschwisterkind': (
      <div key="geschwisterkind" className="space-y-1.5">
        <Label htmlFor="geschwisterkind">{fieldLabel('kinder', 'geschwisterkind')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="geschwisterkind"
            checked={!!fields.geschwisterkind}
            onCheckedChange={(v) => setFields(f => ({ ...f, geschwisterkind: !!v }))}
          />
          <Label htmlFor="geschwisterkind" className="font-normal">{fieldLabel('kinder', 'geschwisterkind')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "besonderheiten": "Besonderheiten (Allergien, Förderbedarf)", "geschwisterkind": "Geschwisterkind bereits in einer Einrichtung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KINDER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
