...
import type { Mitarbeiter, Einrichtungen, LookupValue } from '@/types/app';
...
export type MitarbeiterDialogDefaults = Omit<Mitarbeiter['fields'], 'rolle'> & {
...
export function MitarbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, einrichtungenList, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'rolle', 'email_ma'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "rolle": LookupValue | null, // Rolle (select one key: "traeger" | "einrichtungsleitung") mapping: traeger=Träger (Stadtverwaltung), einrichtungsleitung=Einrichtungsleitung\n  "einrichtung": string | null, // Display name from Einrichtungen (see <available-records>)\n  "email_ma": string | null, // E-Mail\n  "telefon_ma": string | null, // Telefon\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="rolle" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'traeger'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'traeger' ? undefined : 'traeger') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'traeger'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'traeger') ?? 'Träger (Stadtverwaltung)'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'einrichtungsleitung'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'einrichtungsleitung' ? undefined : 'einrichtungsleitung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'einrichtungsleitung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'einrichtungsleitung') ?? 'Einrichtungsleitung'}
          </button>
        </div>
    ...
    'email_ma': (
      <div key="email_ma" className="space-y-1.5">
        <Label htmlFor="email_ma">{fieldLabel('mitarbeiter', 'email_ma')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email_ma"
          type="email"
          inputMode="email"
          placeholder="z. B. anna@forchheim.de"
          value={fields.email_ma ?? ''}
          onChange={e => setFields(f => ({ ...f, email_ma: e.target.value }))}
          required
        />
        {showErrors && !fields.email_ma && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeiter', 'email_ma')}</p>
        )}
      </div>
    ),
    'telefon_ma': (
      <div key="telefon_ma" className="space-y-1.5">
        <Label htmlFor="telefon_ma">{fieldLabel('mitarbeiter', 'telefon_ma')}</Label>
        <Input
          id="telefon_ma"
          type="tel"
          inputMode="tel"
          placeholder="z. B. 09191/123456"
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "rolle": "Rolle", "einrichtung": "Einrichtung", "email_ma": "E-Mail", "telefon_ma": "Telefon"};
